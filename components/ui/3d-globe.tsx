...
function AnimatedArc({
  ...
  const progressRef = useRef(animationOffset % 1);
  ...
  const curve = useMemo(() => {
    ...
    for (let i = 0; i <= 80; i++) {
      ...
      p.normalize().multiplyScalar(radius + Math.sin(Math.PI * t) * arcHeight);
      pts.push(p);
    }
    return new THREE.CatmullRomCurve3(pts);
  }, [from.lat, from.lng, to.lat, to.lng, radius, arcHeight]);

  useFrame((_, delta) => {
    progressRef.current = (progressRef.current + delta * 0.28) % 1;
    ...
    const segments = 60;
    const startIdx = Math.floor(tail * segments);
    const endIdx = Math.floor(head * segments);
    const count = Math.max(2, endIdx - startIdx);

    const pts: THREE.Vector3[] = [];
    for (let i = 0; i <= count; i++) {
      const t = (startIdx + i) / segments;
      pts.push(curve.getPoint(Math.min(t, 1)));
    }
    if (pts.length < 2) return;

    const newGeo = new THREE.TubeGeometry(
      new THREE.CatmullRomCurve3(pts), count, 0.005, 6, false
    );
  ...
  const initialGeometry = useMemo(() => {
    const pts = [curve.getPoint(0), curve.getPoint(0.01)];
    return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), 2, 0.005, 6, false);
  }, [curve]);
...
function ArcArrow({ to, radius, color = "#f0a030" }: ArcArrowProps) {
  const position = useMemo(
    () => latLngToVector3(to.lat, to.lng, radius * 1.02),
    [to.lat, to.lng, radius]
  );
...
function PulseRing({ marker, radius, color = "#e85a2b" }: PulseRingProps) {
  const meshRef = useRef<THREE.Mesh>(null);
  ...
  useFrame((_, delta) => {
    if (!meshRef.current) return;
    scaleRef.current = (scaleRef.current + delta * 0.8) % 2;
    meshRef.current.scale.setScalar(1 + scaleRef.current);
    (meshRef.current.material as THREE.MeshBasicMaterial).opacity =
      Math.max(0, 0.6 - scaleRef.current * 0.3);
  });
...
function Marker({ marker, radius, defaultSize, onClick, onHover }: MarkerProps) {
  ...
  const isOrigin = marker.isOrigin;
  const markerPx = isOrigin ? 32 : 26;

  return (
    <group
      ref={groupRef} visible={isVisible}>
      {/* Pin stem — original gray cylinder */}
      <mesh position={lineCenter} quaternion={lineQuaternion}>
        <cylinderGeometry args={[0.006, 0.006, lineHeight, 8]} />
        <meshBasicMaterial
        ...
        >
          <div
            className={cn(
              ...
              hovered && "scale-100 shadow-xl ring-1 ring-white/50",
            )}
            style={{
              width: "8px",
              height: "8px",
...
function RotatingGlobe({
  ...
  const markerByLabel = useMemo(() => {
    const map: Record<string, GlobeMarker> = {};
    markers.forEach((m) => { if (m.label) map[m.label] = m; });
    return map;
  }, [markers]);

  const arrowDestinations = useMemo(() => {
    ...
    >
      ...
      <mesh geometry={geometry}>
        <meshStandardMaterial
          map={earthTexture}
          bumpMap={bumpTexture}
          bumpScale={config.bumpScale * 0.01}
          roughness={0.4}
          metalness={0.1}
        />
      </mesh>
      ...
      {connections.map((conn, i) => {
        const fromMarker = markerByLabel[conn.from];
        const toMarker = markerByLabel[conn.to];
        ...
        return (
          <AnimatedArc
            ...
            color={conn.color ?? "#f0a030"}
            arcHeight={conn.arcHeight ?? 0.42}
            animationOffset={conn.animationOffset ?? i * 0.18}
          />
        );
      })}

      {/* Arrowheads at destinations */}
      {arrowDestinations.map((conn, i) => {
        const toMarker = markerByLabel[conn.to];
        if (!toMarker) return null;
        return (
          <ArcArrow
            key={`arrow-${i}`}
            to={toMarker}
      ...
      {/* Pulse rings on origin markers */}
      {pulseMarkers.map((m, i) => (
        <PulseRing key={`pulse-${i}`} marker={m} radius={config.radius} color="#e85a2b" />
      ))}
...
function Scene({
  ...
  React.useEffect(() => {
    camera.position.set(0, 0, config.radius * 2.2);
    camera.lookAt(0, 0, 0);
    ...
    <div className={cn("relative h-[80vh] w-full", className)}>
      <Canvas
        ...
        camera={{
          ...
          near: 0.1,
          far: 1000,
          position: [0, 0, mergedConfig.radius * 2.2],
        }}
